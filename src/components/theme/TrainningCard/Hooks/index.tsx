...
export default function useRouterTimer() {
  ...
  const routerPushTimer = (value: string) => router.push(pathname + '?' + createQueryString('timer', value))

  const routerTimer = (value: KeyType) => {
    const getTimer = searchParams.get('timer')

      if (!getTimer && value !== '0' && value !== '00' && value !== 'Backspace') return routerPushTimer(value)

      if (getTimer && getTimer.length < 4 && getTimer.length > 0 && value !== 'Backspace') return routerPushTimer(getTimer+value)
  
      if(value === 'Backspace' && getTimer!.length > 0) return routerPushTimer( getTimer!.slice(0, -1))
  }
  ...
  const keyDownEvent = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const key = (isNumber() ? 'default' : e.key) as keyof typeof mapKeyboardPress
